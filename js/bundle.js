    ...
    function getStaticInformation(selector, activeClass) {
        ...
        elements.forEach(el => {
            el.addEventListener('click', (e) => {
                let targetSex = e.target.getAttribute('id')
                let targetRatio = e.target.getAttribute('data-ratio');
                
                if (targetRatio) {
                    ratio = +targetRatio;
                    localStorage.setItem('ratio', +targetRatio);
                } else {
                    sex = targetSex;
                    localStorage.setItem('sex', targetSex);
                }
...
function cards() {
    class MenuCard {
        ...
        changeToUAH() {
            this.price = this.price * this.tranfser;
        }
        ...
        render() {
            ...
            element.innerHTML = `
                ...
                <div class="menu__item-price">
                    ...
                    <div class="menu__item-total"><span>${this.price}</span> hryvna/day</div>
                </div>
...
function forms(formSelector, modalTimerId) {
    ...
    // Создаем модальное окно с благодарностью
...
function modal(triggerSelector, modalSelector, modalTimerId) {
    ...
    modal.addEventListener('click', (e) => {
        if (e.target === modal || e.target.getAttribute('data-close' == '')) {
            closeModal(modalSelector); // => здесь уже вызываем функцию
        }
    });
...
function slider({container, slide, nextArrow, prevArrow, totalCounter, currentCounter, wrapper, field}) {
    ...
    // slider.appendChild(carouselIndicators);
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    (0,_modules_timer__WEBPACK_IMPORTED_MODULE_2__["default"])('.timer', '2023-03-31');
